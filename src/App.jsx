...
function App() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#101014] via-[#181824] to-[#232336] text-gray-100 font-sans flex flex-col items-center">
        ...
        <div className={`flex-1 flex justify-center md:justify-end md:pr-4 mb-8 md:mb-0 transition-all duration-1000 ${aboutVisible ? 'animate-fadeInLeft delay-300' : 'opacity-0 -translate-x-10'}`}>
          <div className="relative group">
            <img 
              ...
              loading="eager"
              fetchPriority="high"
              className="w-48 h-48 sm:w-72 sm:h-72 md:w-96 md:h-96 lg:w-[28rem] lg:h-[28rem] object-cover rounded-full border-4 border-yellow-400 shadow-2xl bg-[#181824] transition-all duration-500 group-hover:scale-105 hover:animate-pulse-gentle animate-float" 
              style={{objectPosition: 'center 20%'}} 
            />
            <div className="absolute -top-6 -left-6 sm:-top-8 sm:-left-8 w-12 h-12 sm:w-20 sm:h-20 bg-yellow-400 rounded-full z-0 opacity-90 animate-pulse-slow" />
            <div className="absolute -bottom-4 -right-4 w-8 h-8 sm:w-12 sm:h-12 bg-yellow-400/50 rounded-full z-0 animate-float delay-1000" />
          </div>
        </div>
        {/* Right: About Me Title & Info */}
        <div className={`flex-1 flex flex-col items-center md:items-start text-center md:text-left md:pl-0 transition-all duration-1000 ${aboutVisible ? 'animate-fadeInRight delay-500' : 'opacity-0 translate-x-10'}`}>
          <div className="flex items-center gap-3 mb-2">
            <span className="inline-block w-8 h-1 bg-yellow-400 rounded-full animate-pulse" />
            <span className="uppercase text-sm tracking-widest text-yellow-400 font-semibold">About Me</span>
          </div>
          <h1 ref={typingRef} className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold tracking-tight text-white leading-tight mb-2">
            <span className="inline-block">
              {typedText}<span className="animate-pulse text-yellow-400">|</span>
            </span>
          </h1>
          ...
          <div className={`flex gap-6 mb-8 transition-all duration-700 ${aboutVisible ? 'animate-fadeInUp delay-700' : 'opacity-0'}`}>
            {SOCIALS.map((s, index) => (
              <a 
                key={s.name} 
                href={s.url} 
                className="text-gray-300 hover:text-yellow-400 transition-all duration-300 hover:scale-125 hover:-translate-y-2 hover:rotate-6 animate-pulse-gentle"
                style={{fontSize:'2.2rem', animationDelay: `${0.8 + index * 0.1}s`}} 
                aria-label={s.name}
                target="_blank"
      ...
      {/* Education Section */}
      <section ref={aboutRef} id="education" className={`w-full max-w-4xl mx-auto py-10 sm:py-14 md:py-16 px-2 sm:px-4 bg-[#181824] rounded-2xl shadow-lg border-2 border-yellow-400/20 relative mt-4 hover-lift transition-all duration-700 ${aboutVisible ? 'animate-fadeInUp' : 'opacity-0 translate-y-10'}`}>
        {/* Background */}
        <div className="absolute inset-0 z-0 rounded-2xl overflow-hidden pointer-events-none">
          ...
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-2/3 h-32 bg-yellow-400/10 blur-2xl rounded-full animate-pulse-slow" />
          <div className="absolute bottom-0 right-0 w-1/3 h-24 bg-yellow-400/10 blur-2xl rounded-full animate-pulse-slow delay-1000" />
        </div>
        <div className="relative z-10">
          <h2 className={`text-center text-3xl sm:text-4xl font-extrabold text-yellow-400 mb-2 animate-float tracking-wide transition-all duration-700 ${aboutVisible ? 'animate-fadeInUp delay-300' : 'opacity-0'}`}>
            Education
          </h2>
          <div className="flex flex-col items-center mb-8">
            <span className="inline-block w-16 h-1 bg-yellow-400 rounded-full mb-2 animate-pulse" />
            <span className="text-lg text-white font-semibold tracking-wide">My Academic Journey</span>
          </div>
          <div className="relative flex flex-col md:flex-row gap-8 md:gap-0">
            <div className="flex-1 flex flex-col gap-16 md:gap-24">
              {/* High School */}
              <div className={`flex flex-col md:flex-row md:items-center gap-4 transition-all duration-700 ${aboutVisible ? 'animate-fadeInLeft delay-500' : 'opacity-0'}`}>
                ...
                <div className="hidden md:block w-0.5 h-24 bg-gray-600 mx-4 rounded-full relative">
                  <span className="absolute left-1/2 -translate-x-1/2 top-0 w-3 h-3 bg-yellow-400 rounded-full animate-pulse" />
                </div>
                <div className="md:w-1/2" />
              </div>
              {/* University */}
              <div className={`flex flex-col md:flex-row md:items-center gap-4 transition-all duration-700 ${aboutVisible ? 'animate-fadeInRight delay-700' : 'opacity-0'}`}>
                <div className="md:w-1/2" />
                <div className="hidden md:block w-0.5 h-24 bg-gray-600 mx-4 rounded-full relative">
                  <span className="absolute left-1/2 -translate-x-1/2 top-0 w-3 h-3 bg-yellow-400 rounded-full animate-pulse" />
                </div>
      ...
      {/* Skills Section */}
      <section id="skills" className={`scroll-mt-32 w-full max-w-4xl mx-auto bg-[#181824] rounded-2xl shadow-lg p-4 sm:p-8 md:p-10 mt-4 flex flex-col items-center border-2 border-yellow-400/20 relative overflow-hidden hover-lift transition-all duration-700 ${aboutVisible ? 'animate-fadeInUp delay-200' : 'opacity-0 translate-y-10'}`}>
        <div className="absolute inset-0 z-0 rounded-2xl overflow-hidden pointer-events-none">
          <div className="w-full h-full bg-gradient-to-br from-[#232336] via-[#181824] to-[#101014] opacity-90" />
        ...
        <div className="relative z-10 w-full flex flex-col items-center">
          <h2 className={`text-center text-3xl sm:text-4xl font-extrabold text-yellow-400 mb-2 animate-float transition-all duration-700 ${aboutVisible ? 'animate-fadeInUp delay-400' : 'opacity-0'}`}>Skills</h2>
          <div className="flex flex-col items-center mb-8">
            <span className="inline-block w-16 h-1 bg-yellow-400 rounded-full mb-2 animate-pulse" />
            <span className="text-lg text-white font-semibold tracking-wide">My Technical Arsenal</span>
          </div>
          <div className="w-full flex flex-col md:flex-row gap-6 md:gap-0">
            {/* Tech Stacks */}
            <div className={`flex-1 flex flex-col items-center border-b md:border-b-0 md:border-r border-[#232336] pb-6 md:pb-0 md:pr-8 transition-all duration-700 ${aboutVisible ? 'animate-fadeInLeft delay-600' : 'opacity-0'}`}>
              ...
              <ul className="text-base sm:text-lg font-semibold text-blue-200 space-y-1 text-center">
                <li className="hover:text-yellow-400 transition-colors duration-300">Python</li>
                <li className="hover:text-yellow-400 transition-colors duration-300">C#</li>
                <li className="hover:text-yellow-400 transition-colors duration-300">Java</li>
                <li className="hover:text-yellow-400 transition-colors duration-300">HTML, CSS, JavaScript</li>
              </ul>
            </div>
            {/* Tools */}
            <div className={`flex-1 flex flex-col items-center md:pl-8 pt-6 md:pt-0 transition-all duration-700 ${aboutVisible ? 'animate-fadeInRight delay-800' : 'opacity-0'}`}>
              ...
              <ul className="text-base sm:text-lg font-semibold text-blue-200 space-y-1 text-center">
                <li className="hover:text-yellow-400 transition-colors duration-300">Figma</li>
                <li className="hover:text-yellow-400 transition-colors duration-300">Git, GitHub</li>
                <li className="hover:text-yellow-400 transition-colors duration-300">Apache NetBeans</li>
                <li className="hover:text-yellow-400 transition-colors duration-300">.NET MAUI</li>
                <li className="hover:text-yellow-400 transition-colors duration-300">Visual Studio Code</li>
                <li className="hover:text-yellow-400 transition-colors duration-300">Visual Studio 2022</li>
              </ul>
            </div>
          </div>
        </div>
      ...
      {modalOpen && createPortal(
        ...
        >
          ...
          >
            <button
              onClick={closeModal}
              className="absolute top-2 right-2 bg-[#232336] text-gray-200 hover:text-yellow-400 focus:text-yellow-300 rounded-full p-2 shadow-lg focus:outline-none z-10 border-2 border-yellow-400/30 transition-all duration-300 hover:scale-110"
              aria-label="Close"
              type="button"
            >
      ...
      <section id="project" className={`w-full flex justify-center py-10 sm:py-14 md:py-16 px-2 sm:px-4 transition-all duration-700 ${aboutVisible ? 'animate-fadeInUp delay-400' : 'opacity-0 translate-y-10'}`}>
        <div className="w-full max-w-6xl flex flex-col items-center">
          <h2 className="text-center text-3xl sm:text-4xl font-extrabold text-yellow-400 mb-2 animate-float">Project</h2>
          <div className="flex flex-col items-center mb-8">
            <span className="inline-block w-16 h-1 bg-yellow-400 rounded-full mb-2 animate-pulse" />
            <span className="text-lg text-white font-semibold tracking-wide">My Creative Works</span>
          </div>
                ...
                <div className="relative z-10 w-full flex flex-col items-center h-full">
                  ...
                  <div className="flex flex-col items-center text-center w-full">
                    ...
                    {p.title === 'ETSU Portfolio Website' && (
                      <a href="https://etsuwithtea.github.io/etsu-portfolio/" target="_blank" rel="noopener noreferrer" className="text-green-400 text-sm font-medium mt-1 hover:underline hover:text-green-300 focus:text-green-400 transition-all duration-300 hover:scale-105">View Website →</a>
                    )}
                    {p.title === 'CupCraft Café Management System' && (
                      <a href={p.link} target="_blank" rel="noopener noreferrer" className="text-green-400 text-sm font-medium mt-1 hover:underline hover:text-green-300 focus:text-green-400 transition-all duration-300 hover:scale-105">View Website →</a>
                    )}
                    {p.github && (
                      <a href={p.github} target="_blank" rel="noopener noreferrer" className="text-blue-400 text-sm font-medium mt-1 hover:underline hover:text-blue-300 focus:text-blue-400 transition-all duration-300 hover:scale-105">View Source Code (GitHub) →</a>
                    )}
                  </div>
                </div>
              </div>
      ...
      {/* Contact Section */}
      <section id="contact" className={`w-full max-w-3xl mx-auto py-10 sm:py-14 md:py-16 px-2 sm:px-4 transition-all duration-700 ${aboutVisible ? 'animate-fadeInUp delay-500' : 'opacity-0 translate-y-10'}`}>
        <h2 className="text-center text-3xl sm:text-4xl font-extrabold text-yellow-400 mb-2 animate-float">Contact Me</h2>
        <div className="flex flex-col items-center mb-8">
          <span className="inline-block w-16 h-1 bg-yellow-400 rounded-full mb-2 animate-pulse" />
          <span className="text-lg text-white font-semibold tracking-wide">Get in Touch</span>
        </div>
        <div className={`bg-[#181824] rounded-2xl shadow-lg p-6 sm:p-8 md:p-10 border-2 border-yellow-400/20 flex flex-col gap-6 relative overflow-hidden hover:scale-105 transition-all duration-500 hover:border-yellow-400/80 ${aboutVisible ? 'animate-fadeInUp delay-700' : 'opacity-0'}`}>
          <div className="absolute inset-0 z-0 rounded-2xl overflow-hidden pointer-events-none">
            <div className="w-full h-full bg-gradient-to-br from-[#232336] via-[#181824] to-[#101014] opacity-90" />
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-2/3 h-32 bg-yellow-400/10 blur-2xl rounded-full animate-pulse-slow" />
            <div className="absolute bottom-0 right-0 w-1/3 h-24 bg-yellow-400/10 blur-2xl rounded-full animate-pulse-slow delay-1000" />
          ...
          <div className="relative z-10 w-full flex flex-col gap-6">
            <div className={`flex items-center gap-4 transition-all duration-500 hover:scale-105 hover:-translate-y-1 ${aboutVisible ? 'animate-fadeInLeft delay-900' : 'opacity-0'}`}>
              <svg className="text-yellow-400 animate-pulse" width="28" height="28" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>
              <div>
                <div className="font-bold text-white">Name</div>
                <div className="text-gray-300">Navapan Suthon</div>
              </div>
            </div>
            <div className={`flex items-center gap-4 transition-all duration-500 hover:scale-105 hover:-translate-y-1 ${aboutVisible ? 'animate-fadeInLeft delay-1000' : 'opacity-0'}`}>
              <svg className="text-yellow-400 animate-pulse" width="28" height="28" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M22 16.92V19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2v-2.08a2 2 0 0 1 .91-1.68l3.2-2.13a2 2 0 0 1 2.18 0l2.1 1.4a2 2 0 0 0 2.22 0l2.1-1.4a2 2 0 0 1 2.18 0l3.2 2.13a2 2 0 0 1 .91 1.68z"/>
            ...
            <div className={`flex items-center gap-4 transition-all duration-500 hover:scale-105 hover:-translate-y-1 ${aboutVisible ? 'animate-fadeInLeft delay-1100' : 'opacity-0'}`}>
              <svg className="text-yellow-400 animate-pulse" width="28" height="28" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M4 4h16v16H4z"/><path d="M22 6l-10 7L2 6"/></svg>
              <div>
      ...
      <button
        ...
        className="fixed bottom-4 right-4 sm:bottom-8 sm:right-8 flex items-center gap-2 px-4 py-2 sm:px-5 rounded-full border-2 border-yellow-400/30 bg-[#181824] text-gray-200 font-semibold shadow hover:bg-[#232336] hover:text-yellow-400 focus:ring-2 focus:ring-yellow-400 transition-all duration-300 z-50 text-sm sm:text-base hover:scale-110 hover:-translate-y-2 hover:rotate-3 animate-float"
        aria-label="Back to Top"
      >
        Back to Top
        <span className="w-5 h-5 sm:w-6 sm:h-6 rounded-full border-2 border-yellow-400 flex items-center justify-center bg-[#181824] animate-pulse">
          <svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" className="hover:animate-bounce">
