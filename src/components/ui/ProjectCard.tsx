...
export function ProjectCard({ project, onClick }: ProjectCardProps) {
  return (
    <motion.button
      ...
      className="group/item relative rounded-[2rem] overflow-hidden cursor-pointer shadow-2xl bg-black/5 block w-full text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-glow/60 focus-visible:ring-offset-4 focus-visible:ring-offset-surface"
      style={{
    ...
    >
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover/item:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-8">
        <div className="translate-y-4 group-hover/item:translate-y-0 transition-transform duration-500">
          <p className="text-[10px] uppercase tracking-[0.3em] font-bold text-white/80 mb-2">{project.category}</p>
          <h3 className="text-2xl font-black text-white mb-4 leading-none">{project.title}</h3>
      ...
      {/* Small floating tag */}
      <div className="absolute top-6 right-6 px-3 py-1 rounded-full bg-black/40 border border-white/20 text-[9px] font-bold uppercase tracking-widest text-white">
        {project.category}
      </div>
    </motion.button>
  );
