    ...
    >
      <motion.div layout className="flex-1 flex flex-col relative z-10">
        <div className="flex flex-col gap-2">
          <p className="text-[10px] uppercase tracking-[0.4em] font-bold opacity-70">Get in touch</p>
          <motion.h2 
            layout
            className={cn(
              "font-display font-black tracking-tighter leading-[0.85] transition-all duration-500",
              isActive ? "text-6xl sm:text-7xl md:text-8xl lg:text-9xl mb-8" : "text-4xl sm:text-5xl md:text-6xl"
            )}
          >
            Connect.
          </motion.h2>
          ...
          {isActive && (
            ...
            >
              ...
              <motion.button
                type="button"
                variants={fadeInUp}
                aria-label={`Copy phone number ${contactInfo.phone}`}
                className="group/item relative text-left w-full p-8 rounded-3xl bg-black/[0.04] border border-black/10 hover:bg-black/[0.08] transition-all cursor-pointer flex flex-col justify-between min-h-[160px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black/40 focus-visible:ring-offset-2 focus-visible:ring-offset-accent"
                onClick={(e) => {
                  e.stopPropagation();
              ...
              >
                <div className="flex justify-between items-start">
                  <div className="w-10 h-10 rounded-xl bg-black/10 flex items-center justify-center text-black/70">
                    <Phone size={18} />
                  </div>
                  <AnimatePresence mode="wait">
                    {copied === "phone" ? (
                      ...
                      <Copy size={14} className="opacity-50 group-hover/item:opacity-80 transition-opacity" />
                    )}
                  </AnimatePresence>
                </div>
                <div>
              ...
              >
                <div className="flex justify-between items-start">
                  ...
                  <ArrowUpRight size={16} className="opacity-50 group-hover/item:opacity-80 transition-opacity" />
                </div>
                <div>
                  <p className="text-[10px] uppercase tracking-widest font-bold opacity-70 mb-1">Email</p>
      ...
      <motion.div layout className="flex justify-between items-center mt-8 relative z-10">
        ...
        <div className="flex items-center gap-4">
          <div className="p-3 rounded-full bg-black/5 border border-black/10 text-black/70">
