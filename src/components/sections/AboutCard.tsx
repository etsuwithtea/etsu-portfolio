    ...
    >
      <motion.div layout className="flex-1 flex flex-col justify-center relative">
        ...
        <h2 className={cn(
          ...
          {isActive ? (
            <>
              {t("common.hiIm")} <span style={{ color: "var(--color-secondary-fg)" }}>{PERSONAL_INFO.name}</span>
              <br />
              <span className="text-3xl opacity-60 font-sans tracking-normal block mt-4 font-medium">
                {t("common.creativeDeveloper")}
              </span>
            </>
          ) : (
            language === "en" ? (
              ...
              <>ทำความรู้จัก <span style={{ color: "var(--color-secondary-fg)" }}>กับตัวฉัน</span></>
            )
          )}
        </h2>
          ...
          >
            ...
            <div className="md:col-span-7 space-y-12">
              <motion.div variants={{ hidden: { opacity: 0, x: -20 }, visible: { opacity: 1, x: 0 } }}>
                <p className="text-xl md:text-2xl leading-relaxed font-light" style={{ color: "var(--color-neutral-300)" }}>
                  {t("personal.bio", PERSONAL_INFO.bio)}
                </p>
              </motion.div>
              ...
              >
                "{t("personal.quote", PERSONAL_INFO.quote)}"
              </motion.div>
              ...
              >
                {[
                  { label: t("common.ageLabel"), value: t("personal.age", PERSONAL_INFO.age) },
                  { label: t("common.statusLabel"), value: t("personal.status", PERSONAL_INFO.status) },
                  { label: t("common.interestsLabel"), value: t("personal.interests", PERSONAL_INFO.interests) },
                ].map((detail, idx) => (
            ...
            <div className="md:col-span-5 space-y-10">
              ...
              <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }}>
                <h4 className="text-xs uppercase tracking-[0.3em] font-bold mb-6 opacity-40">
                ...
                <div className="space-y-8">
                  {EXPERIENCE.map((exp, i) => (
                    <div key={i} className="group cursor-default">
                      ...
                      <p className="text-sm font-medium mb-3" style={{ color: "var(--color-primary-glow)" }}>
                        {t(`experience.${i}.role`, exp.role)}
                      </p>
                      <p className="text-xs opacity-60 whitespace-pre-line leading-relaxed max-w-sm">
                        {t(`experience.${i}.desc`, exp.desc)}
                      </p>
                    </div>
                  ))}
                </div>
                  ...
                  {EDUCATION.map((edu, i) => (
                    <div key={i}>
                      ...
                      <h5 className="text-xl font-bold">
                        {t(`education.${i}.degree`, edu.degree)}
                      </h5>
                      <p className="text-sm opacity-80 mt-1">
                        {t(`education.${i}.school`, edu.school)}
                      </p>
                      {edu.desc && (
                        <p className="text-xs opacity-50 mt-2 italic">
                          {t(`education.${i}.desc`, edu.desc)}
                        </p>
                      )}
                    </div>
                  ))}
      ...
      <motion.div layout className="flex justify-between items-end mt-4">
        <p className="text-lg" style={{ color: "var(--color-neutral-300)" }}>
          {isActive ? t("common.close") : t("common.aboutMe")}
        </p>
        {isActive
