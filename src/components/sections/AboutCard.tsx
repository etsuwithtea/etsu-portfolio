          ...
          >
            ...
            <div className="md:col-span-7 space-y-12">
              ...
              <motion.blockquote
                ...
                className="relative italic text-2xl md:text-3xl leading-snug"
                style={{ color: "var(--color-secondary-fg)" }}
              >
                <span
                  aria-hidden="true"
                  className="block font-display not-italic text-6xl leading-[0.4] mb-3"
              ...
              >
                {[
                  ...
                  <div key={idx} className="px-5 py-2 bg-white/[0.07] rounded-full border border-white/10">
                    <span className="text-[10px] uppercase tracking-widest opacity-65 block mb-0.5">{detail.label}</span>
            ...
            <div className="md:col-span-5 space-y-10">
              ...
              <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }}>
                <h4 className="text-xs uppercase tracking-[0.3em] font-bold mb-6 opacity-60">Work Experience</h4>
                <div className="space-y-8">
                  {EXPERIENCE.map((exp, i) => (
                    <div key={i} className="group cursor-default">
                      <p className="text-xs font-mono opacity-60 mb-2">{exp.period}</p>
              ...
              {/* Education */}
              <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }}>
                <h4 className="text-xs uppercase tracking-[0.3em] font-bold mb-6 opacity-60">Education</h4>
                <div className="space-y-8">
                  {EDUCATION.map((edu, i) => (
                    <div key={i}>
                      <p className="text-xs font-mono opacity-60 mb-2">{edu.year}</p>
                      <h5 className="text-xl font-bold">{edu.degree}</h5>
                      <p className="text-sm opacity-80 mt-1">{edu.school}</p>
                      {edu.desc && <p className="text-xs opacity-65 mt-2 italic">{edu.desc}</p>}
                    </div>
                  ))}
                </div>
              </motion.div>
