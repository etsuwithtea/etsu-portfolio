...
export const getSkillIcon = (skill: string, isDarkMode = false): JSX.Element => {
  const iconMap: Record<string, JSX.Element> = {
    ...
    'Next.js': <i className={`fab fa-react ${isDarkMode ? 'text-white' : 'text-zinc-900'}`}></i>,
    'Node.js': <i className="fab fa-node-js text-green-500"></i>,
    Express: <i className={`fas fa-server ${isDarkMode ? 'text-zinc-400' : 'text-zinc-700'}`}></i>,
    ...
    'Socket.IO': <i className={`fas fa-plug ${isDarkMode ? 'text-white' : 'text-gray-900'}`}></i>,
